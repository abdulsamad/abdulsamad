...
const RepositorySchema = z.object({
  ...
  homepageUrl: z.url().nullable().transform((value) => value ?? ''),
  openGraphImageUrl: z.url().nullable().transform((value) => value ?? ''),
...
const technologies = new Set([
  'html', 'css', 'scss', 'javascript', 'typescript', 'react', 'css-in-js', 'material-ui', 'dayjs',
  'vite', 'emotion', 'content-api', 'mongoosejs', 'localforage', 'monaco-editor', 'react-split',
  'styled-components', 'xterm', 'mongodb', 'pwa', 'node', 'nodejs', 'open-weather-map-api', 'axios',
  'superagent', 'redux', 'redux-toolkit', 'react-spring', 'famer-motion', 'react-router-dom',
  'nextjs', 'gatsby', 'reach-router', 'alpine-js', 'bulma', 'jspdf', 'workbox',
]);
